...
def init_database():
    ...
    if not existing_admin:
        ...
        db.execute("""
            INSERT INTO admins
            (username, full_name, password_hash, active, role_id)
            VALUES (?, ?, ?, ?, ?)
        """, (
            "admin",
...
def generate_registration_number():
    ...
    return f"WM-{year}-{highest + 1:04d}"
...
def register():
    if request.method == "POST":
        jina_kamili = request.form.get("jina_kamili", "").strip()
        makazi = request.form.get("makazi", "").strip()
        jinsia = request.form.get("jinsia", "").strip()
        namba_ya_sim = request.form.get("namba_ya_sim", "").strip()
        ...
        if file and file.filename:
            ...
            picha_filename = f"{uuid.uuid4().hex}.{extension}"
...
def dashboard():
    ...
    active_admins = db.execute(
        "SELECT COUNT(*) AS total FROM admins WHERE active = 1"
    ).fetchone()["total"]
...
def activate_admin(id):
    ...
    db.execute(
        "UPDATE admins SET active = 1 WHERE id = ?",
        (id,)
    )
...
def deactivate_admin(id):
    ...
    db.execute(
        "UPDATE admins SET active = 0 WHERE id = ?",
        (id,)
    )
